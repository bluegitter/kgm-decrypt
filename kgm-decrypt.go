...
var (
	VprHeader   = [16]byte{0x05, 0x28, 0xBC, 0x96, 0xE9, 0xE4, 0x5A, 0x43, 0x91, 0xAA, 0xBD, 0xD0, 0x7A, 0xF5, 0x36, 0x31}
	KgmHeader   = [16]byte{0x7C, 0xD5, 0x32, 0xEB, 0x86, 0x02, 0x7F, 0x4B, 0xA8, 0xAF, 0xA6, 0x8E, 0x0F, 0xFF, 0x99, 0x14}
	VprMaskDiff = [17]byte{0x25, 0xDF, 0xE8, 0xA6, 0x75, 0x1E, 0x75, 0x0E, 0x2F, 0x80, 0xF3, 0x2D, 0xB8, 0xB6, 0xE3, 0x11, 0x00}
	Table1      = [272]byte{
...
func decrypt(data []byte, isVpr bool) []byte {
	headerLen := binary.LittleEndian.Uint32(data[0x10:0x14])
	key := make([]byte, 17)
...
func main() {
	if len(os.Args) < 2 {
		fmt.Println("Usage: kgm-decrypt <directory-path>")
		return
	}
	...
	for _, file := range files {
		if strings.HasSuffix(file.Name(), ".kgm") {
			inputPath := filepath.Join(dir, file.Name())
			outputPath := strings.TrimSuffix(inputPath, ".kgm") + ".mp3"

			data, err := ioutil.ReadFile(inputPath)
			if err != nil {
				fmt.Println("Error reading file:", inputPath, err)
				continue
			}

			isVpr := false // Modify this based on your file type detection
			decryptedData := decrypt(data, isVpr)

			err = ioutil.WriteFile(outputPath, decryptedData, 0644)
			if err != nil {
				fmt.Println("Error writing file:", outputPath, err)
				continue
			}

			fmt.Println("Decrypted:", inputPath, "->", outputPath)
		}
	}
}
